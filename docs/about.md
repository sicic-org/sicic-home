---
description: About Super Individuals CIC — our public-interest mission, Community Interest Company structure, founding team, and compliance.
---

# About

## What Super Individuals CIC Is

Super Individuals CIC is a course and membership community focused on building real, owned AI agent systems. We bring together professionals, builders, and curious minds, teach them to build hands-on, then give them a long-term community in which to keep building.

We exist to help individuals become **AI-native builders**, not just consumers of AI — and to do it in the open, with public proof of work.

<div class="programme-panel programme-panel--narrow programme-panel--soft cpd-panel">
  <img class="cpd-member-logo" src="../assets/cpd-member-logo.jpg" alt="CPD Member, The CPD Certification Service">
  <div>
    <p class="programme-lead"><strong>CPD status:</strong> Super Individuals CIC is a member of The CPD Certification Service (Membership No. {{ cpd_membership_no }}).</p>
  </div>
</div>

## Our Social Purpose

Super Individuals is a **Community Interest Company (CIC)** registered in England and Wales. A CIC is a company created specifically to serve a community rather than private shareholders.

Our purpose is public-interest: to widen access to practical, domain-specific AI capability so that individuals — not only large institutions — can build and deliver things that matter.

<div class="grid-2">
  <div class="feature-card">
    <h3>Asset lock for the mission</h3>
    <p>As a CIC, our assets and any surplus are subject to a statutory <strong>asset lock</strong>: they are dedicated to our social purpose and cannot be extracted as private profit. If the company is ever dissolved, remaining assets pass to another asset-locked, public-interest body.</p>
  </div>
  <div class="feature-card">
    <h3>Why this matters for partners</h3>
    <p>For organisations working with us on training or CSR-aligned programmes, the CIC structure means investment in our work stays committed to community benefit — and supports applications for grants and mission-aligned funding.</p>
  </div>
</div>

## The Team

<div class="grid-2">
  <div class="feature-card">
    <div class="cohort-member-head">
      <h3>Yiju Jia</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/hailan867/" aria-label="Yiju Jia LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Director</p>
    <p>Background in business and executive education within the Oxford Saïd alumni network, where the first SICIC workshop was framed. </p>
  </div>
  <div class="feature-card">
    <div class="cohort-member-head">
      <h3>Dr. Bijun Li</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/dr-bijun-li/" aria-label="Dr. Bijun Li LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Director · Technical Direction</p>
    <p>Holds a PhD in cybersecurity from Germany. As an entrepreneur, leads SICIC's technical direction with a focus on security and on building safe, robust AI applications.</p>
  </div>
</div>

## What We Do

- [{{ course_name }}](course.md) — a live, online, two-week course of {{ course_hours }} structured learning hours, delivered in two modules
- cohort-based learning with public proof of work
- an invitation-only membership community
- team training and build engagements for organisations ([For Teams](teams.md))

## Compliance

- **Legal form:** Community Interest Company (CIC), registered in England and Wales
- **Company registration number:** {{ company_number }}
- **Registered office:** {{ registered_office }}
- **CPD:** member organisation of The CPD Certification Service (Membership No. {{ cpd_membership_no }})
- **Course bookings and cancellations:** [{{ register_email }}](mailto:{{ register_email }})
- **General contact:** [{{ general_email }}](mailto:{{ general_email }})
- **Policies:** [Booking Terms & Cancellation Policy](booking-terms.md) · [Terms of Service](terms-of-service.md) · [Privacy Policy](privacy-policy.md) · [Refund Policy](refund-policy.md)
