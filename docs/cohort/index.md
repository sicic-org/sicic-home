# Cohort

## Overview

Our cohorts focus on building real AI applications.
Participants work on projects involving AI agents, automation, and practical use cases.

## Founding Cohort

The founding cohort is the first membership group inside Super Individuals.
These early members are building in public, shipping real work, and establishing the tone for what the community becomes.

Projects are built within a limited timeframe. The focus is on execution, not perfection.

## Founding Cohort Snapshot

<div class="grid-3">
  <div class="proof-card">
    <p class="proof-label">Membership Tier</p>
    <h3>Founding Cohort</h3>
    <p>The first 31 members of the community.</p>
  </div>
  <div class="proof-card">
    <p class="proof-label">What Matters</p>
    <h3>Build in public</h3>
    <p>Member pages and live demos make the cohort's work visible.</p>
  </div>
  <div class="proof-card">
    <p class="proof-label">Why It Exists</p>
    <h3>Execution over theory</h3>
    <p>These pages document what early members actually shipped.</p>
  </div>
</div>

## Member Demos

<div class="cohort-member-grid">
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Gemma Oldfield</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/gemma-oldfield-680b342b/" aria-label="Gemma Oldfield LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Masters AI for Business Oxford | MA | Software Delivery | Project Delivery | Programme Management | Change Maker | Product Delivery | Scrum Master | University Creative Industries Advisory Board Member</p>
    <p class="pricing-meta">My Digital Lab</p>
    <p>A ZeroClaw-based lab for designing agent workflows and turning raw thinking into structured output.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="goldfield-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://gjoldfield-01.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Mafalda Patricio</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/mafaldaiapatricio/" aria-label="Mafalda Patricio LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Transforming the UK Comms, Media & High Tech @ Salesforce ☁️ | WISAwards Finalist 2024</p>
    <p class="pricing-meta">Mafalda's Digital Lab</p>
    <p>A ZeroClaw-based lab documenting agent architecture, methodology, and content intelligence workflows.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="patricio-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://mafaldapatricio.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Aijie Shu</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/aijie-shu-5420047a/" aria-label="Aijie Shu LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">AI, Blockchain, Full-Stack Developer</p>
    <p class="pricing-meta">HackStudio Pro</p>
    <p>An AI video production studio for turning research into bilingual, broadcast-ready YouTube documentaries from a single codebase.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="shu-hackstudio-pro/">Member Page</a>
      <a class="md-button" href="https://losdwind.github.io/hackstudio-lite/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Yiwen Tuo</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/yiwen-tuo-646243342/" aria-label="Yiwen Tuo LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">PhD Candidate | Visiting Researcher @ Trinity College Dublin</p>
    <p class="pricing-meta">Paper Party</p>
    <p>A research conversation tool that turns uploaded papers into mapped literature context and dialogue.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="tuo-paper-party/">Member Page</a>
      <a class="md-button" href="https://paper-party.vercel.app/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Jaidev Venkatesan</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/jaidev-venkatesan-11190012" aria-label="Jaidev Venkatesan LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Senior Manager - Enabling Data &amp; AI Governance</p>
    <p class="pricing-meta">My ZeroClaw Lab</p>
    <p>A ZeroClaw lab where Nova handles YouTube content intelligence through Telegram, MCP tooling, and a custom fetch pipeline.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="venkatesan-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://j-ai-dev.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Gigi Palma</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/gigirodriguezpalma/" aria-label="Gigi Palma LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Strategic Program &amp; Transformation Leader | Scaling Global Teams, Risk &amp; Operations | PMP | Oxford MSc</p>
    <p class="pricing-meta">Gp's ZeroClaw Lab</p>
    <p>A personal agent experiment lab documenting Nova's architecture, weekly learnings, and a real estate analyzer showcase.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="palma-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://gijirpox26.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Bruno Chikeka</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/brunochikeka/" aria-label="Bruno Chikeka LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Systems Architect | AI x Infrastructure x Institutional Design | MSc Oxford Said</p>
    <p class="pricing-meta">Bruno's Bricolage Lab</p>
    <p>An agentic protocol for discovering, verifying, and assembling free-tier resources into working outcomes.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="chikeka-bricolage/">Member Page</a>
      <a class="md-button" href="https://chikeka.github.io/bricolage-portfolio/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Luis Loio</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/luis-loio/" aria-label="Luis Loio LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Consulting | Treasury &amp; Investments | Oxford Executive MBA</p>
    <p class="pricing-meta">Luis'ZeroClaw Lab</p>
    <p>An early ZeroClaw lab scaffold with architecture, troubleshooting, and showcase sections ready to be developed.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="loio-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://aiagencycs.github.io/My-ZeroClaw-Agent/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Charlotte Luzuka</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/charlotte-luzuka" aria-label="Charlotte Luzuka LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Senior PM | General Operator | Digital Health &amp; Innovation Leader | Angel Investor | Women in Tech Advocate</p>
    <p class="pricing-meta">CHIP — Charlie's Padel Coach</p>
    <p>A padel coaching agent that turns matches, clips, and player context into clear priorities and structured improvement plans.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="luzuka-chip/">Member Page</a>
      <a class="md-button" href="https://charliebnl.github.io/Charlotte-chip-lab/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Jamaal Linton</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/jamaal-linton-13b242b0" aria-label="Jamaal Linton LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Sales Engineer | Datasite</p>
    <p class="pricing-meta">J's ZeroClaw Lab</p>
    <p>A zero-to-deployed agent build documenting Nova, the VPS setup, and the operational blockers solved along the way.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="linton-zeroclaw/">Member Page</a>
      <a class="md-button" href="https://jamaal-l.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Dr. Vivek Kumar</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/vivek-kumar-phd-44011683/" aria-label="Dr. Vivek Kumar LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Research Scientist | SMIEEE | Fellow SCRS | AI &amp; Mental Health</p>
    <p class="pricing-meta">NewsAware</p>
    <p>A newsroom agent for checking sentiment, credibility, and factual reliability in high-stakes public claims.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="kumar-newsaware/">Member Page</a>
      <a class="md-button" href="https://vsrana-ai.github.io/NewsAware/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Lucile Ripa</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/lucileripa/" aria-label="Lucile Ripa LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">CEO &amp; Founder @FanePath AI | AI Visibility for Brands | Innovative AI Solutions</p>
    <p class="pricing-meta">FanePath AI Lab</p>
    <p>A ZeroClaw-based lab for exploring AI visibility for brands, agent workflows, and practical brand-facing AI systems.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="ripa-fanepath/">Member Page</a>
      <a class="md-button" href="https://lucile-fp.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Nigel Shardlow</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/shardlow/" aria-label="Nigel Shardlow LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Marketing Science &amp; Data Strategy Leader | Independent Advisor</p>
    <p class="pricing-meta">Nigel's Agent Lab</p>
    <p>A multi-agent lab for marketing science and data strategy, with Genie and Bin Vader selected for the public showcase.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="shardlow-agents/">Member Page</a>
      <a class="md-button" href="https://sharkstar-code.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Johannes Erlingsson</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/johanneserlingsson/" aria-label="Johannes Erlingsson LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">AI Specialist and consultant | Developing GenAI solutions and enabling organisational capability | AI @ Oxford</p>
    <p class="pricing-meta">agent-character-creator</p>
    <p>A platform-agnostic skill for turning agent persona design into reusable SOUL, IDENTITY, USER, TOOLS, and ROUTINES files.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="erlingsson-agent-character/">Member Page</a>
      <a class="md-button" href="https://github.com/urbanmania/skills">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Berfu Simsek</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/berfu-simsek/" aria-label="Berfu Simsek LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Customer Experience &amp; Growth Expert | Senior Business Leader | Non-Executive Director | FMCG &amp; Retail | Circular Economy | Executive MBA</p>
    <p class="pricing-meta">Priority Task Guide</p>
    <p>An agent concept for turning instructions into a full to-do list, proposing next steps, and guiding work by urgency.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="simsek-task-agent/">Member Page</a>
      <a class="md-button" href="https://github.com/brfsm">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Rachel Pope</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/rachel-e-pope/" aria-label="Rachel Pope LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">CFO &amp; Finance Leader | Sovereign &amp; Portfolio Finance | Cambridge Maths &bull; Oxford MBA</p>
    <p class="pricing-meta">Rachel's Finance AI Lab</p>
    <p>A ZeroClaw-based lab for finance leadership, sovereign and portfolio finance, and strategic decision support workflows.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="pope-finance-lab/">Member Page</a>
      <a class="md-button" href="https://rachelepope.github.io/my-zeroclaw-agents/">Demo</a>
    </div>
  </div>
  <div class="proof-card cohort-member-card">
    <p class="cohort-badge">Founding Cohort</p>
    <div class="cohort-member-head">
      <h3>Ilija Tozija</h3>
      <a class="cohort-linkedin" href="https://www.linkedin.com/in/ilijatozija/" aria-label="Ilija Tozija LinkedIn profile">
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path d="M19 3A2 2 0 0 1 21 5V19A2 2 0 0 1 19 21H5A2 2 0 0 1 3 19V5A2 2 0 0 1 5 3H19M8.34 17V9.67H5.9V17H8.34M7.12 8.66A1.41 1.41 0 1 0 7.12 5.84A1.41 1.41 0 0 0 7.12 8.66M18.1 17V12.96C18.1 10.8 16.95 9.53 15.42 9.53C14.19 9.53 13.64 10.21 13.33 10.69V9.67H10.89C10.92 10.35 10.89 17 10.89 17H13.33V12.91C13.33 12.69 13.35 12.47 13.41 12.31C13.58 11.87 13.97 11.41 14.6 11.41C15.42 11.41 15.75 12.03 15.75 12.94V17H18.1Z"/>
        </svg>
      </a>
    </div>
    <p class="cohort-member-role">Infrastructure &amp; Energy | Program | Risk, Governance &amp; Economics</p>
    <p class="pricing-meta">Project coming soon</p>
    <p>Founding member profile published. Public project page and demo will be added when ready.</p>
    <div class="cohort-card-actions">
      <a class="md-button md-button--primary" href="tozija-profile/">Member Page</a>
    </div>
  </div>
</div>

## What These Pages Show

Each founding cohort card is designed to scale as more demos are published. Over time this page will become a compact public record of what the first **31 members** built.
